...
type AppRouteResponses<T extends AppRoute, TStatus extends HTTPStatusCode, TClientOrServer extends 'client' | 'server'> = {
...
}[keyof T['responses'] & TStatus] | (Exclude<TStatus, keyof T['responses']> extends never ? never : {
    status: Exclude<TStatus, keyof T['responses']>;
    body: unknown;
});
export type ServerInferResponses<T extends AppRoute | AppRouter, TStatus extends HTTPStatusCode = HTTPStatusCode> = T extends AppRoute ? Prettify<AppRouteResponses<T, TStatus, 'server'>> : T extends AppRouter ? {
    [TKey in keyof T]: ServerInferResponses<T[TKey], TStatus>;
} : never;
export type ClientInferResponses<T extends AppRoute | AppRouter, TStatus extends HTTPStatusCode = HTTPStatusCode> = T extends AppRoute ? Prettify<AppRouteResponses<T, TStatus, 'client'>> : T extends AppRouter ? {
...
export type ClientInferRequest<T extends AppRoute | AppRouter> = T extends AppRoute ? Prettify<Without<{
    params: [undefined] extends PathParamsWithCustomValidators<T, 'client'> ? never : Prettify<PathParamsWithCustomValidators<T, 'client'>>;
    body: T extends AppRouteMutation ? T['contentType'] extends 'multipart/form-data' ? FormData | ZodInputOrType<T['body']> : ZodInputOrType<T['body']> : never;
